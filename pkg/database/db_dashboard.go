...
func (d *DbManager) ListAllDashboards() ([]Dashboard, error) {
    ...
    query := "SELECT dashboard_id, dashboard_title, dashboard_content FROM dashboard;"
    rows, err := d.Connection.Query(d.context, query)
    if err != nil {
        return nil, err
    }
...
func (d *DbManager) GetDashboard(dashID int) (Dashboard, error) {
    var dash Dashboard
    query := "SELECT dashboard_id, dashboard_title, dashboard_content FROM dashboard WHERE dashboard_id = $1;"
...
func (d *DbManager) InsertDashboard(dash Dashboard) (int, error) {
    var id int
    query := "INSERT INTO dashboard (dashboard_title, dashboard_content) VALUES ($1, $2) RETURNING dashboard_id;"
...
func (d *DbManager) DeleteDashboard(dashID int) error {
    query := "DELETE FROM dashboard WHERE dashboard_id = $1;"
